...
class Unet(nn.Module):
    ...
    def __init__(self):
        ...
        self.double_conv1 = double_conv(1, start_fm, 3, 1, 2) #72*72*fm
        #Max Pooling 1
        self.maxpool1 = nn.MaxPool2d(kernel_size=2) #36*36*fm

        #Convolution 2
        self.double_conv2 = double_conv(start_fm, start_fm * 2, 3, 1, 2) #36*36*fm*2
        #Max Pooling 2
        self.maxpool2 = nn.MaxPool2d(kernel_size=2) #18*18*fm*2

        #Convolution 3
        self.double_conv3 = double_conv(start_fm * 2, start_fm * 4, 3, 1, 2) #18*18*fm*4
        #Max Pooling 3
        ...
        # One by One Conv
        self.one_by_one = nn.Conv2d(start_fm, 1, 1, 1, 2)
...
def pad(variabledown,variableup, th, tw):
    temp = torch.zeros_like(variableup)
    h, w = variableup.shape[2], variableup.shape[3]  
    x1 = int(round((w - tw) / 2.))
    y1 = int(round((h - th) / 2.))
    temp[:, :, y1 : y1 + th, x1 : x1 + tw]=variabledown
    return temp
