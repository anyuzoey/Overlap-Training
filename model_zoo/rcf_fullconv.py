...
class RCF(nn.Module):
    def __init__(self):
        ...
        self.conv3_3 = nn.Conv2d(128, 128, 3, padding=2)

        self.conv4_1 = nn.Conv2d(128, 256, 3, padding=2)
        self.conv4_2 = nn.Conv2d(256, 256, 3, padding=2)
        self.conv4_3 = nn.Conv2d(256, 256, 3, padding=2)
        ...
        self.conv1_1_down = nn.Conv2d(32, 21, 1, padding=2)
        self.conv1_2_down = nn.Conv2d(32, 21, 1, padding=2)

        self.conv2_1_down = nn.Conv2d(64, 21, 1, padding=2)
        self.conv2_2_down = nn.Conv2d(64, 21, 1, padding=2)

        self.conv3_1_down = nn.Conv2d(128, 21, 1, padding=2)
        self.conv3_2_down = nn.Conv2d(128, 21, 1, padding=2)
        self.conv3_3_down = nn.Conv2d(128, 21, 1, padding=2)

        self.conv4_1_down = nn.Conv2d(256, 21, 1, padding=2)
        self.conv4_2_down = nn.Conv2d(256, 21, 1, padding=2)
        self.conv4_3_down = nn.Conv2d(256, 21, 1, padding=2)
        
        self.conv5_1_down = nn.Conv2d(256, 21, 1, padding=2)
        self.conv5_2_down = nn.Conv2d(256, 21, 1, padding=2)
        self.conv5_3_down = nn.Conv2d(256, 21, 1, padding=2)

        #lr 0.01 0.02 decay 1 0
        self.score_dsn1 = nn.Conv2d(21, 1, 1, padding=2)
        self.score_dsn2 = nn.Conv2d(21, 1, 1, padding=2)
        self.score_dsn3 = nn.Conv2d(21, 1, 1, padding=2)
        self.score_dsn4 = nn.Conv2d(21, 1, 1, padding=2)
        self.score_dsn5 = nn.Conv2d(21, 1, 1, padding=2)
        #lr 0.001 0.002 decay 1 0
        self.score_final = nn.Conv2d(5, 1, 1, padding=2)
    ...
    def forward(self, x):
        ...
        conv1_1_down = pad(conv1_1_down,conv1_2_down)
        so1_out = self.score_dsn1(conv1_1_down + conv1_2_down)
        conv2_1_down = pad(conv2_1_down,conv2_2_down)
        so2_out = self.score_dsn2(conv2_1_down + conv2_2_down)
        conv3_1_down = pad(conv3_1_down,conv3_3_down)
        conv3_2_down = pad(conv3_2_down,conv3_3_down)
        so3_out = self.score_dsn3(conv3_1_down + conv3_2_down + conv3_3_down)
        conv4_1_down = pad(conv4_1_down,conv4_3_down)
        conv4_2_down = pad(conv4_2_down,conv4_3_down)
        so4_out = self.score_dsn4(conv4_1_down + conv4_2_down + conv4_3_down)
        conv5_1_down = pad(conv5_1_down,conv5_3_down)
        conv5_2_down = pad(conv5_2_down,conv5_3_down)
        ...
        upsample2 = torch.nn.functional.conv_transpose2d(so2_out, weight_deconv2, stride=2, padding=2)
        upsample3 = torch.nn.functional.conv_transpose2d(so3_out, weight_deconv3, stride=4, padding=2)
        upsample4 = torch.nn.functional.conv_transpose2d(so4_out, weight_deconv4, stride=8, padding=2)
        upsample5 = torch.nn.functional.conv_transpose2d(so5_out, weight_deconv5, stride=8, padding=2)
        ### center crop
...
def pad(variablesmall,variablelarge):
    h, w = variablesmall.shape[2], variablesmall.shape[3] 
    temp = torch.zeros_like(variablelarge)
    th, tw = variablelarge.shape[2], variablelarge.shape[3]  
    x1 = int(round((tw - w) / 2.))
    y1 = int(round((th - h) / 2.))
    temp[:, :, y1 : y1 + h, x1 : x1 + w]=variablesmall
    return temp
